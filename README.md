...
| `--poc "RSpoc,CSpoc,LYEpoc,REpoc,CEpoc,ORDER;..."` | Main-header progression changes. The current writer supports complete schedules on single- and multi-tile grids with one tile-part per tile and inline packet headers; omitted `--tile-parts` is normalized to `none`. |
