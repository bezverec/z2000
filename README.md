...
| `--poc "RSpoc,CSpoc,LYEpoc,REpoc,CEpoc,ORDER;..."` | Main-header progression changes. Complete schedules support one part per tile or `L` parts when each quality layer is contiguous; other divided layouts fail closed. Omitted `--tile-parts` is normalized to `none`. |
