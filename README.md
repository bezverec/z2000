...
| `--poc "RSpoc,CSpoc,LYEpoc,REpoc,CEpoc,ORDER;..."` | Main-header progression changes. Complete schedules support one part per tile and compatible `R`, `L`, `C`, or reference-grid position `P` parts. Omitted `--tile-parts` is normalized to `none`. |
