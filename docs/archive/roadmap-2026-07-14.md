...
1. Harden the landed grayscale and bounded palette paths with checked-in
   ...
   per-component SIZ sampling and reconstructs embedded Kakadu 4:2:0 fixtures
   through reference-grid RPCL merging plus component-local T1/DWT geometry,
   including unequal multi-precinct grids and inline packet-span derivation
   without PLT. Matching nonzero image/tile origins are retained through
   clipped precinct grids and lifting parity on both single- and multi-tile
   RPCL streams. Independent Kakadu four-tile fixtures with and without PLT
   cover zero and 5/3 origins. Canonical sampled RPCL POC now decodes from main
   or first tile-part headers. Explicit origin-anchored nearest-neighbour
   expansion keeps native decode separate from sRGB interleaving and enables
   the bounded JP2-to-TIFF path. Next broaden packed-header, reordered POC,
   sampled encode, and distinct tile-partition-origin profiles.
