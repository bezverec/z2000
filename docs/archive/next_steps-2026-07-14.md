...
Kakadu 4:2:0 decode vertical; subsampled encode, mixed multi-tile/MCT,
...
F3b slices 1-9 (2026-07-14) now audit and reconstruct bounded subsampling:
...
The reference-grid RPCL iterator now also merges unequal component precinct
grids; a checked-in Kakadu 32x32/16x16/16x16 fixture with 8x8 precincts audits
30 packets and reconstructs all samples exactly. Component-local persistent T2
state now derives the same 30 spans without PLT, pinned by a second Kakadu
fixture. The bounded profile requires RPCL/no-MCT/5-3 and inline
packet headers. Slice 5 permits matching nonzero
`XOsiz/YOsiz` and tile origins while retaining clipped reference-grid
precincts through strict decode, with PLT and PLT-less Kakadu fixtures. Next
the same component-local catalog and planar assembly decode four zero-origin
tiles from independent Kakadu fixtures, with and without PLT. The RPCL merge
clamps aligned precinct positions to each tile origin, which preserves ISO
component order on right and lower tile boundaries. Matching nonzero
image/tile origins now flow through the same four-tile path, pinned by two more
Kakadu fixtures at `XOsiz/YOsiz=5/3` with 102 packets and 198 blocks. Next
canonical RPCL POC from the main or first tile-part header is accepted for
single- and multi-tile sampled streams, pinned by Kakadu PLT and PLT-less
fixtures; schedules that reorder sampled packets remain fail-closed. Next
an explicit origin-anchored nearest-neighbour API expands native planes onto
the full reference grid without colour conversion, and the JP2-to-TIFF path
uses checked interleaving for bounded three-component sRGB files. Broaden
subsampled packed-header state next; distinct tile-partition origins, reordered
sampled POC, sampled encode, and non-sRGB conversion remain fail-closed.
