...
| G4 | 60% | Direct resolution reduction, quality-layer-prefix, bounded row-major tile selection, and single-/multi-tile reference-region selection, plus T1 skipping, catalog compaction, and borrowed packet spans | Intra-tile precinct/code-block pruning, remaining tile/colour layouts, incremental input, and row-oriented output |
...
validation. Extending the same selector to single-tile streams does not move
the estimate again: it completes the public selector surface but decodes the
whole tile, so intra-tile pruning and incremental I/O remain substantial and
this is still a bounded promotion.
...
accept the same absolute selector through a shared component-local crop. A
large single tile still requires full-tile T1/DWT work, so that case narrows the
returned window rather than the working set, and input/output are not
incremental yet.
