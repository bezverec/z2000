...
- `decodeLosslessPlanarToSink`, `decodeLosslessTemporaryToSink`, and
  ...
  `tiff.GrayBandWriter`, two-/four-component conversions through
  `tiff.AlphaBandWriter`, and palette streams through a band expansion into
  `tiff.RgbBandWriter`. Every bounded JP2-to-TIFF conversion now streams;
  incremental codestream input and row-oriented synthesis inside a single large
  tile remain open.
...
4. Record OpenJPEG, Grok, and Kakadu disagreement instead of selecting a
   ...
   puts one PLT in each tile's first tile-part covering the whole tile, while
...
5c. Two RGBA layouts found by a `decode-temp-jp2` sweep over common
   `kdu_compress` output still fail closed: irreversible RGBA (`-rate` on an
   RGBA TIFF gives ICT over the colour channels plus an independent alpha,
   rejected by the JP2 COD audit), and resolution reduction of reversible RGBA
   with RCT through the planar API. Multi-tile reversible RGBA itself is done.
...
reserved-`Sqcc` mutation fails before T1 allocation.

The second G2 slice is also complete: a committed Kakadu reversible no-MCT
stream carries effective component decomposition counts 3/2/1, component-sized
COC precinct lists, and matching QCD/QCC band tables. The strict metadata,
packet scheduler, T2/T1 geometry, reduced-catalog compaction, and inverse 5/3
path all consume the component-local level count. All three components match
Kakadu exactly at full resolution and reduction 1 with identical one/eight-
thread output. Reduction above the minimum component level, duplicate COC, and
a divergent transform fail closed. This bounded slice remains single-tile,
no-MCT, reversible, and common-code-block/style only.

The third G2 slice is complete: a second reversible no-MCT Kakadu stream keeps
NL=3 and common 32x32 precincts while assigning component-local 4x4/default,
...
The second G3 marker slice is complete. One main-header `CRG` carries exactly
one checked X/Y registration pair per SIZ component; duplicates, malformed
lengths, and tile-header placement fail closed. The values are exposed on the
native component layout and survive canonical ZRAW round-trip. They remain
rendering metadata in units of 1/65536 of the component sampling step, so Part
1 codestream reconstruction deliberately does not shift the sample grid.
...
1. **`RGN` Maxshift ROI decode and `CRG` registration — complete for the
   bounded decoder.** RGN main/tile inheritance, T1 bitplane accounting,
   coefficient reconstruction, malformed syntax, and independent T.803 raster
...
5. **Legal `POC` schedules across inline, `PPT`, and `PPM` headers — complete
   ...
   deterministic self-repacks. **General multipart POC decode is complete.** Tile-parts
   are treated as consecutive runs of the tile's composed sequence rather than
   having to align with a progression grouping, and POC is accepted in any
...
2. Broaden independent evidence for multi-tile uniform-sampling encode. Kakadu
   8.4.1 reconstructs the one-component stream exactly; the two-component
   layout has z2000 round-trip evidence only, and OpenJPEG/Grok legs are not
   run yet.
