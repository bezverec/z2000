...
| T2 9→10 · Core syntax +1 | **POC tile-part breadth** (one-part plus contiguous `L`/`C` writer/strict decode landed) | Medium (`R`/`P` schedule boundaries) | Medium (progression intervals across part boundaries; narrow path untouched) | Partial: dense 2x2 `C` is pixel-exact through all four decoders; z2000/OpenJPEG/Kakadu also decode dense `L`, while Grok 20.3.6 misdecodes the tested LRCP-to-RPCL schedule from both z2000 and Kakadu |
...
the landed `L`/`C` paths to compatible `R`/`P` parts) or **N6** (biggest unlock but budget it as a
