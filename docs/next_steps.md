...
| T2 9→10 · Core syntax +1 | **POC tile-part breadth** (one-part and layer-contiguous `L` writer/strict decode landed) | Medium (`R`/`C`/`P` schedule boundaries) | Medium (progression intervals across part boundaries; narrow path untouched) | Partial: z2000/OpenJPEG/Kakadu decode single-tile, 2x2-tile one-part, and dense 2x2-tile `L` output pixel-exactly; Grok 20.3.6 misdecodes both z2000- and Kakadu-produced POC schedules |
...
the landed `L` path to compatible `R`/`C`/`P` parts) or **N6** (biggest unlock but budget it as a
