...
| Full JPEG2000 Part 1 codec family | 74 / 100 | Broad Part 1 encode/decode coverage across tiles, progressions, quantization, irreversible profiles, code-block styles, rate allocation, and robust interop. |
...
| Core codestream syntax | 15 | 13 | Multi-tile SIZ/SOT layouts, disabled MCT, LRCP/RLCP progression (single-tile plus the aligned single-layer multi-tile slices), and redundant COC/QCC component markers (main-header COC/QCC accepted only when they byte-replicate the main COD/QCD; genuine per-component overrides plus tile-part COD/QCD and COC/QCC fail closed; malformed COC `Scoc`/SPcoc coding bytes and QCC qstyle fail as invalid in both paths, with shortened COC/QCC length bounded as truncation in the raw reader and invalid at the JP2 boundary) are public profiles with strict-reader coverage. More marker variants (POC/RGN/CRG), arbitrary component layouts, and remaining style combinations remain. |
| Lossless encode profiles | 15 | 12 | Multi-tile v1/v2 (aligned grids, one tile-part per tile) encodes streams OpenJPEG/Grok decode losslessly for RPCL and single-layer RLCP, accepts TERMALL with deterministic strict roundtrip coverage, emits/strict-decodes single-layer LRCP tile packet order, and accepts untargeted multi-layer RPCL. Remaining multi-tile progressions, remaining multi-tile code-block style bits, and stronger rate/layer allocation remain. |
