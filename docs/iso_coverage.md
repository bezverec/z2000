...
| Full JPEG2000 Part 1 codec family | 69 / 100 | Broad Part 1 encode/decode coverage across tiles, progressions, quantization, irreversible profiles, code-block styles, rate allocation, and robust interop. |
...
| Core codestream syntax | 15 | 12 | Multi-tile SIZ/SOT layouts, disabled MCT, LRCP progression, and redundant COC/QCC component markers (accepted only when they byte-replicate the main COD/QCD; genuine per-component overrides fail closed) are public profiles with strict-reader coverage. More marker variants (POC/RGN/CRG), arbitrary component layouts, and remaining style combinations remain. |
...
| Lossy encode/decode | 15 | 8 | ICT/9-7 with scalar-expounded and scalar-derived quantization exists for the narrow single-tile path with OpenJPEG-matching reconstruction; z2000 also decodes foreign OpenJPEG 2.5.4 9/7 lossy JP2s byte-identically to OpenJPEG across a moderate rate ladder (-r 1..8, pinned by an embedded-fixture regression test) and now covers a heavily truncated `-r 10` ISO-MQ fixture with deterministic decode plus an explicit error bound. Strict irreversible QCD decode accepts signalled scalar-expounded/scalar-derived step sizes and uses their mantissas for dequantization, currently pinned by a synthetic mantissa-rewrite gate. Rate-driven layers use global PCRD allocation (J.14) that lands on byte targets within 0.2-0.4 dB of OpenJPEG's allocator at matched sizes. Arbitrary decode, real Grok/Kakadu 9/7 step-table fixtures, and broader reference-relative error-bound validation remain missing. |
