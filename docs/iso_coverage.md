...
| Full JPEG2000 Part 1 codec family | 75 / 100 | Broad Part 1 encode/decode coverage across tiles, progressions, quantization, irreversible profiles, code-block styles, rate allocation, and robust interop. |
...
| Lossless encode profiles | 15 | 12 | Multi-tile v1/v2 (aligned grids, one tile-part per tile) emits all five Part 1 progression orders; OpenJPEG/Grok decode RPCL, single-layer RLCP, and multi-layer PCRL/CPRL losslessly. TERMALL has deterministic strict roundtrip coverage, single-layer LRCP strict-decodes locally, and RPCL/PCRL/CPRL accept untargeted quality layers. Multi-layer LRCP/RLCP, remaining multi-tile code-block style bits, and stronger rate/layer allocation remain. |
