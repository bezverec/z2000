...
| Lossless encode profiles | 15 | 14 | Multi-tile v1/v2 (aligned grids, one tile-part per tile) emits all five Part 1 progression orders with untargeted quality layers; OpenJPEG/Grok decode three-layer LRCP/RLCP/PCRL/CPRL plus the established RPCL profile losslessly. The multi-tile resilience matrix now includes CAUSAL, SEGMARK, RESET+TERMALL, ERTERM+TERMALL, BYPASS+TERMALL, and standalone RESET/ERTERM/ERTERM+RESET with strict deterministic roundtrips; combined CAUSAL+SEGMARK and all three terminated profiles are pixel-exact through OpenJPEG/Grok/Kakadu, and the standalone profiles are pixel-exact through kdu_expand. Tile-aware rate allocation and unsupported BYPASS-combined style modes remain. |
