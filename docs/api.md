...
- checked main-header POC schedules on single- and multi-tile grids with one
  tile-part per tile, plus `L` tile-parts when every quality layer is one
  contiguous schedule range and `C` parts when each RGB component is one
  contiguous range; the writer emits the requested order
...
- Strict decode accepts checked main-header `POC` schedules on single- and
  multi-tile grids with one part per tile or compatible `L`/`C` parts,
  ...
  for every tile; `R` and `P` POC tile-part layouts remain fail-closed.
