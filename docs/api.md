...
- `encodeLosslessSampledPlanarWithOptions(allocator, planes, sampling, options)`
  ...
  rejected because it is exactly the planar profile. A multi-tile all-1
  layout is still accepted here, although `encodeLosslessPlanarWithOptions`
  now encodes common-grid tile grids too, with the full multi-tile option set.
  Kakadu 8.4.1 reconstructs the one-component multi-tile case exactly
...
lossy 9/7 reference spread. A Maxshift `RGN` marker is accepted in the main
header or a first tile-part header, with its length, component index, and
style checked; the codestream decoder applies the shift. Signed/mixed palettes, arbitrary auxiliary-channel mappings, and
