...
- checked main-header POC schedules on single- and multi-tile grids with one
  ...
  contiguous schedule range; the writer emits the requested order
...
- Strict decode accepts checked main-header `POC` schedules on single- and
  multi-tile grids with one part per tile or compatible `L` layer parts,
  ...
  for every tile; `R`, `C`, and `P` POC tile-part layouts remain fail-closed.
