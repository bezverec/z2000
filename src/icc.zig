...
/// Builds the narrow ICC v4 matrix/identity-TRC profile understood by this
/// module. Matrix columns describe linear source RGB primaries in PCSXYZ D50.
pub fn buildLinearRgbProfile(allocator: std.mem.Allocator, matrix: Matrix) ![]u8 {
    if (invertMatrix(matrix) == null) return IccError.InvalidProfile;
    for (matrix) |row| for (row) |value| {
        if (!std.math.isFinite(value) or value < -32768.0 or value >= 32768.0) {
            return IccError.InvalidProfile;
        }
    };

    const xyz_offsets = [3]usize{ 204, 224, 244 };
    const curve_offset: usize = 264;
    const profile_size: usize = 276;
    const bytes = try allocator.alloc(u8, profile_size);
    errdefer allocator.free(bytes);
    @memset(bytes, 0);

    writeU32(bytes, 0, profile_size);
    bytes[8] = 4;
    bytes[9] = 0x30;
    @memcpy(bytes[12..16], "scnr");
    @memcpy(bytes[16..20], "RGB ");
    @memcpy(bytes[20..24], "XYZ ");
    @memcpy(bytes[36..40], "acsp");
    writeS15Fixed16(bytes, 68, 0.9642) catch return IccError.InvalidProfile;
    writeS15Fixed16(bytes, 72, 1.0) catch return IccError.InvalidProfile;
    writeS15Fixed16(bytes, 76, 0.8249) catch return IccError.InvalidProfile;
    @memcpy(bytes[80..84], "z2k ");

    writeU32(bytes, tag_table_offset, 6);
    const xyz_signatures = [_][]const u8{ "rXYZ", "gXYZ", "bXYZ" };
    const trc_signatures = [_][]const u8{ "rTRC", "gTRC", "bTRC" };
    for (0..3) |channel| {
        const xyz_record = tag_record_offset + channel * tag_record_bytes;
        @memcpy(bytes[xyz_record .. xyz_record + 4], xyz_signatures[channel]);
        writeU32(bytes, xyz_record + 4, xyz_offsets[channel]);
        writeU32(bytes, xyz_record + 8, 20);
        const trc_record = tag_record_offset + (channel + 3) * tag_record_bytes;
        @memcpy(bytes[trc_record .. trc_record + 4], trc_signatures[channel]);
        writeU32(bytes, trc_record + 4, curve_offset);
        writeU32(bytes, trc_record + 8, 12);

        const xyz_offset = xyz_offsets[channel];
        @memcpy(bytes[xyz_offset .. xyz_offset + 4], "XYZ ");
        writeS15Fixed16(bytes, xyz_offset + 8, matrix[0][channel]) catch return IccError.InvalidProfile;
        writeS15Fixed16(bytes, xyz_offset + 12, matrix[1][channel]) catch return IccError.InvalidProfile;
        writeS15Fixed16(bytes, xyz_offset + 16, matrix[2][channel]) catch return IccError.InvalidProfile;
    }
    @memcpy(bytes[curve_offset .. curve_offset + 4], "curv");

    // Make sure the generated bytes round-trip through the strict reader.
    _ = try parseMatrixTrcProfile(bytes);
    return bytes;
}
...
fn writeU32(bytes: []u8, offset: usize, value: anytype) void {
    const converted: u32 = @intCast(value);
    bytes[offset] = @intCast(converted >> 24);
    bytes[offset + 1] = @intCast((converted >> 16) & 0xff);
    bytes[offset + 2] = @intCast((converted >> 8) & 0xff);
    bytes[offset + 3] = @intCast(converted & 0xff);
}

fn writeS15Fixed16(bytes: []u8, offset: usize, value: f64) !void {
    if (!std.math.isFinite(value) or value < -32768.0 or value >= 32768.0) {
        return IccError.InvalidProfile;
    }
    const scaled = @round(value * 65536.0);
    const signed: i32 = @intFromFloat(scaled);
    writeU32(bytes, offset, @as(u32, @bitCast(signed)));
}
